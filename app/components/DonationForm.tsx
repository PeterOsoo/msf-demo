...
export default function DonationForm() {
  ...
  const isFormValid = (() => {
    ...
    if (formData.paymentMethod === "card") {
      ...
      if (!/^(0[1-9]|1[0-2])\/([0-9]{2})$/.test(formData.expiry.trim()))
        return false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add a small deliberate delay for Card processing modal visibility, matching M-Pesa's cadence
      const delay = formData.paymentMethod === "card" ? 1500 : 0;
  ...
  return (
    <>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Tip: Use 404 for Gateway Error, or 408 for M-Pesa Timeout.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label
                className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${formData.paymentMethod === "mpesa" ? "border-red-600 bg-red-50 text-red-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
              >
                <input
                  type="radio"
                  name="paymentMethod"
              ...
              <label
                className={`flex items-center justify-center p-3 border rounded-lg cursor-pointer transition-all ${formData.paymentMethod === "card" ? "border-red-600 bg-red-50 text-red-700" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}
              >
                <input
                  type="radio"
                  name="paymentMethod"
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center space-y-4">
            ...
            <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg mt-2">
              <p className="text-xs text-slate-600 font-medium">
                Please enter your M-Pesa PIN on your phone to confirm the
                payment of{" "}
            ...
            <div className="flex flex-col items-center justify-center pt-4">
              <svg
                className="animate-spin h-6 w-6 text-emerald-600 mb-2"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
              >
      ...
      {/* CREDIT/DEBIT CARD SECURITY INTERACTION SIMULATION MODAL */}
      {status.loading && formData.paymentMethod === "card" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center space-y-4">
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              Encrypting attributes for account holder <br />
              <span className="font-bold text-slate-900 text-base">
                {formData.cardName}
              </span>
              * Tokenizing gateway credentials...
            </p>
            <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg mt-2">
              <p className="text-xs text-slate-600 font-medium">
                Authorizing processing channel allocation requests for{" "}
                <span className="font-bold text-red-600">
                  KSh {formData.amount}
                </span>{" "}
                over PCI-DSS compliant tunnels.
              </p>
            </div>
          </div>
        </div>
